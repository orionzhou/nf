...
def reformat_design(DesignFile,ReadMappingFile,ControlMappingFile):
    ...
    while True:
        ...
        if line:
            ...
            group,replicate,fastQFiles,antibody,control,spots = lspl[0],lspl[1],[x for x in lspl[2:-3] if x],lspl[-3],lspl[-2],lspl[-1]

            ## CHECK VALID NUMBER OF COLUMNS PER SAMPLE
            numCols = len(lspl)
            if numCols not in [7]:
                print("{}: Invalid number of columns (should be 7)!\nLine: '{}'".format(ERROR_STR,line.strip()))
                sys.exit(1)
            ...
            sampleMappingDict[group][replicate].append([fastQFiles, spots])
    ...
    fout.write(','.join(['sample_id','spots','fastq_1','fastq_2']) + '\n')
    for group in sorted(sampleMappingDict.keys()):
        ...
        for replicate in sorted(sampleMappingDict[group].keys()):
            for idx in range(len(sampleMappingDict[group][replicate])):
                fastQFiles, spots = sampleMappingDict[group][replicate][idx]

                ## GET SAMPLE_ID,FASTQ_1,FASTQ_2 COLUMNS
                sample_id = "{}_R{}_T{}".format(group,replicate,idx+1)
                oList = [sample_id, spots] + fastQFiles
                if len(fastQFiles) == 1:
                    oList += ['']
